...
def test_kpred(b):
    ...
    while True:
        if len(ReadRoutine().sensors.list_s[0].a[1]) >= Sensors.MAX_X:
            time.sleep(0.1)
            ReadRoutine().sensors.list_s[0].a[2].getxyFFT(
                ReadRoutine().sensors.rtc)
            KPredictior().append_predict(
                ReadRoutine().sensors.list_s[0].a[2].get_last_fft())
            print(KPredictior().get_prob())
        else:
            print("dsada")
...
def main():
    ...
    b.connect_bt(list_devices, "ACELEROMETROS")
    # test_save_routine(b)
    # test_acsition(b)
    test_kpred(b)
